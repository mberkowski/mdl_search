...
module MDL
  describe BorealisAudioPlayer do
    ...
    it 'produces a configuration for Audios' do
      ...
      expect(viewer(audio)['height']).to eq '125px'
      expect(viewer(audio)['width']).to eq '560px'
    ...
    it 'produces a configuration for an Audio playlist' do
      ...
      expect(viewer(audio_playlist)['height']).to eq '395px'
      expect(viewer(audio_playlist)['width']).to eq '560px'
